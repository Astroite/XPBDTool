// ============================================================================
// [MODULE 4] UI COMPONENTS (UI is same as previous step, kept concise)
// ============================================================================




const Sidebar = ({ solverParams, setSolverParams, matParams, setMatParams, viewParams, setViewParams }: any) => {
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                ...
                {openSections.solver && (
                    <div className="p-4 bg-gray-900/30 space-y-3 border-b border-gray-800">
                        <NumberInput label="Substeps" value={solverParams.substeps} min={1} max={20} step={1} onChange={(v: number) => setSolverParams((p: any) => ({...p, substeps: v}))} />
                        <NumberInput label="Iterations" value={solverParams.iterations} min={1} max={10} step={1} onChange={(v: number) => setSolverParams((p: any) => ({...p, iterations: v}))} />
                        <NumberInput label="Delta Time" value={solverParams.dt} min={0.001} max={0.05} step={0.001} onChange={(v: number) => setSolverParams((p: any) => ({...p, dt: v}))} />
                        <div className="pt-2 mt-2 border-t border-gray-800/50">
                            ...
                            <input type="number" value={solverParams.gravity[1]} className="w-full bg-gray-950 border border-gray-700 rounded py-1 px-2 text-xs text-right text-gray-300" onChange={(e) => setSolverParams((p: any) => ({...p, gravity: [0, parseFloat(e.target.value), 0]}))} />
                        </div>
                    </div>
                )}
                ...
                {openSections.rendering && (
                    <div className="p-4 bg-gray-900/30 space-y-3 border-b border-gray-800">
                        <div className="flex items-center justify-between cursor-pointer" onClick={() => setViewParams((p: any) => ({...p, showWireframe: !p.showWireframe}))}>
